...
public class MenuFactory implements MapMenuFactory {
    ...
    private MapMenuWidget addedSubmenuButton(final MapItem mapItem) {
        ...
        if (null != menuWidget) {
            ...
            IMapMenuWidget submenuWidget = null;
            for (MapWidget child : menuWidget.getChildWidgets()) {
                if (child instanceof MapMenuButtonWidget) {
                    ...
                    submenuWidget = buttonWidget.getSubmenu();
            ...
            if (null != submenuWidget) {
                ...
                float span = 0;
                float width = 0;
                for (IMapWidget child : submenuWidget.getChildren()) {
                    if (child instanceof MapMenuButtonWidget) {
                ...
                final int count = submenuWidget.getChildWidgetCount();
                span /= count;
                width /= count;
                ...
                final int index = submenuWidget.getChildWidgetCount() / 2;
                submenuWidget.addChildWidgetAt(index, buttonWidget);
            }
        }
        return menuWidget;
    }
    ...
    private MapMenuButtonWidget createButton(String iconFile,
        ...
        buttonWidget.setSubmenu(submenu);
        return buttonWidget;
    }
    ...
    private MapMenuButtonWidget createButton(String iconFile) {
        MapMenuButtonWidget buttonWidget = new MapMenuButtonWidget(appContext);
        buttonWidget.setOnButtonClickHandler(new CancelAction());
        buttonWidget.setIcon(createWidgetIcon(iconFile));
        return buttonWidget;
    }
}
